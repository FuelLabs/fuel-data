    ...
    ($subject:ident) => {
        impl From<$crate::Identifier> for $subject {
            fn from(identifier: $crate::Identifier) -> Self {
                match identifier {
                    $crate::Identifier::Address(tx_id, index, id) => $subject::build(
                        Some(tx_id),
                        Some(index),
                        Some($crate::IdentifierKind::Address),
                        Some(id),
                    ),
                    $crate::Identifier::ContractID(tx_id, index, id) => $subject::build(
                        Some(tx_id),
                        Some(index),
                        Some($crate::IdentifierKind::ContractID),
                        Some(id),
                    ),
                    $crate::Identifier::AssetID(tx_id, index, id) => $subject::build(
                        Some(tx_id),
                        Some(index),
                        Some($crate::IdentifierKind::AssetID),
                        Some(id),
                    ),
                    $crate::Identifier::PredicateID(tx_id, index, id) => $subject::build(
                        Some(tx_id),
                        Some(index),
                        Some($crate::IdentifierKind::PredicateID),
                        Some(id),
                    ),
                    $crate::Identifier::ScriptID(tx_id, index, id) => $subject::build(
                        Some(tx_id),
                        Some(index),
